<?php
	...
	if(isset($data_back->{"o_v_id"}) && isset($data_back->{"startdate"}) && isset($data_back->{"enddate"}))
	{
		if(!empty($data_back->{"o_v_id"}) && !empty($data_back->{"startdate"}) && !empty($data_back->{"enddate"}))
		{
			...
			$query = "select o_v_id,v_name,v_no,o_v_kilometer,o_cost,o_maintenance,o_m_cost,o_date,(o_cost+o_m_cost) as 'MTotal' from oilchange,vehicle where o_v_id = v_id and o_v_id = '".$o_v_id."' and o_date between '".$startdate."' and '".$enddate."'";
			$stm = $conn->prepare($query);
			if ($stm)
			{
				$stm->execute();
				$stm->bind_result($o_v_id,$v_name,$v_no,$o_v_kilometer,$o_cost,$o_maintenance,$o_m_cost,$o_date,$Total);
				$stm->store_result();
				$count1=$stm->num_rows;
				...
				if($count1!=0)
				{
					...
					while($stm->fetch())
					{
						$vehicle[]=array('o_v_id'=>"$o_v_id",'v_name'=>"$v_name",'v_no'=>"$v_no",'o_v_kilometer'=>"$o_v_kilometer",'o_cost'=>"$o_cost",'o_maintenance'=>"$o_maintenance",'o_m_cost'=>"$o_m_cost",'o_date'=>"$o_date",'Total'=>"$Total");
					}
