<?php
	...
	if(isset($data_back->{"c_d_id"})&& isset($data_back->{"c_date"}))
	{
		if(!empty($data_back->{"c_d_id"})&& !empty($data_back->{"c_date"}))
		{
			...
			$query = "select c_id,c_dshift,d_name,c_c_name,c_cost,c_cash,c_gascredit,c_gascash,c_maintenance,c_commission,c_gst,c_cashleft,c_total,c_date from driver,dailycashout where c_d_id = d_id and c_d_id = '".$c_d_id."' and c_date = '".$c_date."'";
