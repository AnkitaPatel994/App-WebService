<?php
	...
	if(isset($data_back->{"c_dshift"})&& isset($data_back->{"c_date"}))
	{
		if(!empty($data_back->{"c_dshift"})&& !empty($data_back->{"c_date"}))
		{
			$c_dshift=$data_back->{"c_dshift"};
			$c_date=$data_back->{"c_date"};
			
			$query = "select c_id,c_dshift,d_name,c_c_name,c_cost,c_cash,c_gascredit,c_gascash,c_maintenance,c_commission,c_gst,c_cashleft,c_total,c_date from driver,dailycashout where c_d_id = d_id and c_dshift = '".$c_dshift."' and c_date = '".$c_date."'";
